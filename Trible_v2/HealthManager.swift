...
//        healthStore.execute(squery)
    ...
    func fetchActiveEnergyBurned() async {
            let typeEnergy = HKQuantityType(.activeEnergyBurned)
            let descriptor = HKSampleQueryDescriptor(
                predicates:[.quantitySample(type: typeEnergy)],
                sortDescriptors: [SortDescriptor(\.endDate, order: .reverse)],
                limit: 1)
            ...
            do {
                ...
                DispatchQueue.main.async {
                    if !results.isEmpty {
                        ///TODO: DEV PURPOSES ONLY, REMOVE - 20
                        self.activeEnergyBurned = results.first!.quantity.doubleValue(for: .kilocalorie())
                    }
                }
            } catch {
                print("error fetching active energy burned", error)
            }
        }
}
